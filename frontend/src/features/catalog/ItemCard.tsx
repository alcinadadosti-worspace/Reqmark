...
import { itemSurfaceId, staggerItem } from '@/lib/motion';
...
export const ItemCard = memo(function ItemCard({ item, occupancy, today, onOpen }: ItemCardProps) {
  ...
  return (
    <motion.div variants={staggerItem} className="h-full">
      {/*
        Camada so para o shared layout: o `layoutId` fica separado do wrapper do
        stagger porque as duas animacoes disputam o mesmo `transform` — juntas,
        a entrada da lista faz o card tremer ao abrir.
      */}
      <motion.div layoutId={reduced ? undefined : itemSurfaceId(item.id)} className="h-full">
